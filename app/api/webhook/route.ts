...
export async function POST(req: NextRequest) {
  ...
  switch (event.type) {
    ...
    // ✅ Handle subscription cancellations (grace period)
    case "customer.subscription.deleted": {
      const subscription = event.data.object as Stripe.Subscription;

      console.log("⚠️ Subscription Expired:", JSON.stringify(subscription));

      // 🔍 Find the user subscription in Supabase
      const { error: updateError } = await supabase
        .from("subscriptions")
        .update({ status: "expired" }) // ✅ Mark as expired after grace period
